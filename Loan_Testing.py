...
def LoanScraper():
    ...
    try:
        ...
        headers = {"User-Agent": "LoanBot/1.11"}
        ...
        res = requests.get(f"https://oauth.reddit.com/r/borrow/comments/11vx0mr/", headers=headers).json()
        ...
        for post in load["data"]["children"]:
            df1 = pd.DataFrame({"Username": [post["data"]["author"]], "Return_Amount": [post["data"]["title"]],
                                ...
                                "Return Date": [post["data"]["created"]], "ROI Daily": "", "ROI Monthly": "", "Days":""
            ...
            if amount:
                df1["Req_Amount"].values[0] = int(amount)
            else:
                continue
            ...
            if "USA" or "US" or "U.S" or "U.S.A" in df1["Location"].values[0]:
                v = v.strftime("%m/%d/%y")
            elif "China" in df1["Location"].values[0] or "Japan" in df1["Location"].values[0] or "Korea" in df1["Location"].values[0] or "Iran" in df1["Location"].values[0]:
                v = v.strftime("%y/%m/%d")
            else:
                v = v.strftime("%d/%m/%y")
            ...
            # Calculate ROI.
            df1["ROI Daily"].values[0] = (((df1["Return_Amount"].values[0] - df1["Req_Amount"].values[0])/df1["Req_Amount"].values[0]) * 100) / days
            ...
            gods_discord_channel = "https://discord.com/api/webhooks/1046397300492226562/3relRHjYSFwRcg_4X8CiULni_FKuOk9NpaMoaAghoWj-ucL7a2MG3i_v8R5b35QjYHIN"
            test_server_channel = "https://discord.com/api/webhooks/1046484935806034111/ibjPR77pKPqh_bhmVuC0d0XwGXEwMTlcbCWszj4Nz2Qt_CHWXnBcAksvzmLbb8lDRSQ6"
            welcome_test_server_channel = "https://discord.com/api/webhooks/1055498127618097174/prxoD_ZW0vO6ghrwZNhOiwI9AeLBfNYYss6MTLPkjgvAb_B3WJgWNLMMtPWYZ67GTYbn"

            discord_url = test_server_channel
...
def send_discord_message(df1, post_title, post_url, currency, discord_url):
    ...
    embed.set_thumbnail(url="https://styles.redditmedia.com/t5_33lr0/styles/communityIcon_ibpbtkoanvh01.png?width=256&s=be19b3a03070dbfcc68cb1fcf7022d24102ad3a6")
...
def parse_comment(loan_comment_id, headers, counter):
    ...
    if not 'paid as a borrower' in comment_body:
        paid_loans, total_borrowed, unpaid_loans, total_owed, in_progress_loans, outstanding_loans = parse_table(author, comment_body)
    else:
...
def send_edited_discord_message(df1, post_title, post_url, currency, discord_url, sending_id):
    ...
    edit.set_thumbnail(url="https://styles.redditmedia.com/t5_33lr0/styles/communityIcon_ibpbtkoanvh01.png?width=256&s=be19b3a03070dbfcc68cb1fcf7022d24102ad3a6")
